...
socketSignals.on('heartbeat', async (hb) => {
	...
	if (hbPrintControl > 9) {
		hbPrintControl = 0;
		console.log(" ")
		console.log("初心 gainsCUBE server remains live at: " + (new Date).toISOString() + " 初心")
...
socketSignals.on("signals", async (signal) => {
	...
	if (openTrade === true) {
		...
		var tx = {
			...
			gasPrice: web3[selectedProvider].utils.toHex(offChainGasPrice.instant * parseFloat(process.env.GAS_PRICE_MULTIPLIER) * 1e9),
			gas: web3[selectedProvider].utils.toHex("6400000"),
			gasLimit: web3[selectedProvider].utils.toHex("12800000")
		};
